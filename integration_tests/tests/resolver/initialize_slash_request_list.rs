#[cfg(test)]
mod tests {
    use resolver_core::slash_request_list::SlashRequestList;

    use crate::fixtures::fixture::TestBuilder;

    #[tokio::test]
    async fn test_initialize_slasher_request_list_ok() {
        ...
        resolver_program_client
            .do_initialize_slash_request_list(&ncn_root.ncn_admin, &ncn_root.ncn_pubkey)
            .await
            .unwrap();

        let slash_request_list: SlashRequestList = resolver_program_client
            .get_account(
                &SlashRequestList::find_program_address(
                    &resolver_program::id(),
                    &ncn_root.ncn_pubkey,
                )
                .0,
        ...
        assert_eq!(slash_request_list.ncn, ncn_root.ncn_pubkey);
        assert_eq!(slash_request_list.list.len(), 32);
    }
}
